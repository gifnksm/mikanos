...
void DrawBall(uint64_t layer_id, int x, int y) {
  SyscallWinFillRectangle(layer_id, 4 + x - kBallRadius, 24 + y - kBallRadius, 2 * kBallRadius,
                          2 * kBallRadius, 0x007f00);
  SyscallWinFillRectangle(layer_id, 4 + x - kBallRadius / 2, 24 + y - kBallRadius / 2, kBallRadius,
                          kBallRadius, 0x00ff00);
}
...
extern "C" void main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    do {
      if (ball_x_ < kGapWidth || kCanvasWidth - kGapWidth <= ball_x_ || ball_y_ < kGapHeight ||

...
void DrawEye(uint64_t layer_id_flags, int mouse_x, int mouse_y, uint32_t color) {
  ...
  SyscallWinFillRectangle(layer_id_flags, eye_x - kEyeSize / 2, eye_y - kEyeSize / 2, kEyeSize,
                          kEyeSize, color);
}
...
extern "C" void main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (events[0].type == AppEvent::kQuit) {
      ...
      SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 4, 24, kCanvasSize, kCanvasSize,
                              0xffffff);

...
WithError<int> MakeArgVector(char *command, char *first_arg, char **argv, int argv_len,
                             char *argbuf, int argbuf_len) {
  int argc = 0;
  int argbuf_index = 0;
...
Terminal::Terminal(Task &task, const TerminalDescriptor *term_desc) : task_{task} {
  ...
  if (show_window_) {
    window_ = std::make_shared<ToplevelWindow>(kColumns * 8 + 8 + ToplevelWindow::kMarginX,
...
void Terminal::Scroll1() {
  Rectangle<int> move_src{ToplevelWindow::kTopLeftMargin + Vector2D<int>{4, 4 + 16},
...
void Terminal::ExecuteLine() {
  ...
  if (pipe_char) {
    *pipe_char = 0;
    ...
    auto term_desc =
        new TerminalDescriptor{subcommand, true, false, {pipe_fd, files_[1], files_[2]}};
    files_[1] = pipe_fd;
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    for (int i = 0; i < pci::num_device; ++i) {
      ...
      PrintToFd(*files_[1], "%02x:%02x.%d vend=%04x head=%02x class=%02x.%02x.%02x\n", dev.bus,
                dev.device, dev.function, vendor_id, dev.header_type, dev.class_code.base,
                dev.class_code.sub, dev.class_code.interface);
    }
    ...
    if (t.TimeZone == EFI_UNSPECIFIED_TIMEZONE) {
      ...
      PrintToFd(*files_[1], "%d-%02d-%02d %02d:%02d:%02d ", t.Year, t.Month, t.Day, t.Hour,
                t.Minute, t.Second);
      if (t.TimeZone >= 0) {
    ...
    for (int slot = 1; slot < 256; ++slot) {
      ...
      PrintToFd(*files_[1], "Slot %d: ID %04x:%04x Class %d.%d.%d\n", slot,
                dev->DeviceDesc().vendor_id, dev->DeviceDesc().product_id,
                dev->DeviceDesc().device_class, dev->DeviceDesc().device_sub_class,
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry &file_entry, char *command,
                                     char *first_arg) {
  __asm__("cli");
  ...
  while (true) {
    ...
    case Message::kTimerTimeout:
      ...
      if (show_window && window_isactive) {
        ...
        Message msg =
            MakeLayerMessage(task_id, terminal->LayerId(), LayerOperation::DrawArea, area);
        __asm__("cli");
        task_manager->SendMessage(1, msg);
        __asm__("sti");
      }
    ...
    case Message::kKeyPush:
      if (msg->arg.keyboard.press) {
        const auto area = terminal->InputKey(msg->arg.keyboard.modifier, msg->arg.keyboard.keycode,

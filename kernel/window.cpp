...
void Window::SetTransparentColor(std::optional<PixelColor> c) { transparent_color_ = c; }
...
WindowRegion Window::GetWindowRegion(Vector2D<int> pos) { return WindowRegion::kOther; }
...
WindowRegion ToplevelWindow::GetWindowRegion(Vector2D<int> pos) {
  if (pos.x < 2 || Width() - 2 <= pos.x || pos.y < 2 || Height() - 2 <= pos.y) {
    ...
    if (Width() - 5 - kCloseButtonWidth <= pos.x && pos.x < Width() - 5 && 5 <= pos.y &&

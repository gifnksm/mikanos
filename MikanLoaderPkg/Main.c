...
EFI_STATUS OpenRootDir(EFI_HANDLE image_handle, EFI_FILE_PROTOCOL **root) {
  ...
  status = gBS->OpenProtocol(image_handle, &gEfiLoadedImageProtocolGuid, (VOID **)&loaded_image,
...
EFI_STATUS OpenBlockIoProtocolForLoadedImage(EFI_HANDLE image_handle,
  ...
  status =
      gBS->OpenProtocol(loaded_image->DeviceHandle, &gEfiBlockIoProtocolGuid, (VOID **)block_io,
...
EFI_STATUS ReadBlocks(EFI_BLOCK_IO_PROTOCOL *block_io, UINT32 media_id, UINTN read_bytes,
                      VOID **buffer) {
  EFI_STATUS status;
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table) {
  ...
  Print(L"Resolution: %ux%u, Pixel Format: %s, %u pixels/line\n",
        ...
        GetPixelFormatUnicode(gop->Mode->Info->PixelFormat), gop->Mode->Info->PixelsPerScanLine);
  Print(L"Frame Buffer: 0x%0lx - 0x%0lx, Size: %lu bytes\n", gop->Mode->FrameBufferBase,
        gop->Mode->FrameBufferBase + gop->Mode->FrameBufferSize, gop->Mode->FrameBufferSize);
  ...
  if (status == EFI_SUCCESS) {
    ...
    Print(L"Reading %lu bytes (Present %d, BlockSize %u, LastBlock %u)\n", volume_bytes,
          media->MediaPresent, media->BlockSize, media->LastBlock);
